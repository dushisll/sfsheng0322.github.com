...
首先在repo的根目录下面，新建一个名为CNAME的文本文件，里面写入你要绑定的域名，比如sunfusheng.com；如果绑定的是顶级域名，则DNS要新建一条A记录，指向103.245.222.133。如果绑定的是二级域名，则DNS要新建一条CNAME记录，指向username.github.com（请将username换成你的用户名）。此外，别忘了将_config.yml文件中的baseurl改成根目录”/”。
